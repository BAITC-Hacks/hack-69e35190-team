...
1. Нормализовать пробелы в `description`, сохранив оригинальный текст для цитирования. Разбить на законченные короткие фрагменты по границам предложений и пунктов списка. Не обрывать фрагмент посреди слова; длинное предложение можно разбить на смысловые части с сохранением точных смещений в оригинале.
2. Каждому фрагменту назначить `(profile_id, fragment_index, start, end)`. `description[start:end]` должен в точности воспроизводить текст фрагмента. Пустые и явно шаблонные фразы не индексировать.
3. Один раз вычислить эмбеддинги фрагментов выбранной предобученной моделью и сохранить их с версиями модели и CSV. Дообучения нет. При изменении CSV или модели перестроить индекс.
...
- Индекс фрагментов привязан к хешу CSV и версии модели эмбеддингов. Кэшировать эмбеддинг нормализованного запроса; ключ — канонические поля запроса и версия модели. При точном равенстве сходства выбирать меньший индекс фрагмента. Для полной повторяемости текста объяснения кэшировать выбранный фрагмент или использовать только шаблон.
